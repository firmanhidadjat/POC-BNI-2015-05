...
public abstract class U {
	...
	private static final Gson pgson = new GsonBuilder().setPrettyPrinting().create();
	...
	static {
		...
		try {
			PBEKeySpec password = new PBEKeySpec("agus@hanoman.co.id---dodoldurenbakwan".toCharArray(), iv, 1000, 128);
	...
	public static String dump(byte bb[], int offset, int length) {
		...
		if (ascii) {
			try {
				return bb.length+" ["+URLEncoder.encode(new String(bb), "UTF8").replaceAll("\\+", " ")+"]";
			} catch (UnsupportedEncodingException e) {
				return "ERROR<<:"+e.getMessage()+">>";
			}
		} else {
			return bb.length+" HEX ["+toHex(bb)+"]";
		}
	}
...
//			out.println(o.getClass().getName()+" ["+o+"]");
	...
	public static String getValueAsString(JsonObject obj, String name, String nvl) {
		JsonElement p = obj.get(name);
		if (p == null) return nvl;
		return p.getAsString();
	}
	
	public static int getValueAsInt(JsonObject obj, String name, int nvl) {
		JsonElement p = obj.get(name);
		if (p == null) return nvl;
		return p.getAsInt();
	}
	
	public static long getValueAsLong(JsonObject obj, String name, long nvl) {
		JsonElement p = obj.get(name);
		if (p == null) return nvl;
		return p.getAsLong();
	}
	
	public static BigDecimal getValueAsDecimal(JsonObject obj, String name, BigDecimal nvl) {
		JsonElement p = obj.get(name);
		if (p == null) return nvl;
		return new BigDecimal(p.getAsString());
	}
	
	public static Boolean getValueAsBoolean(JsonObject obj, String name, Boolean nvl) {
		JsonElement p = obj.get(name);
		if (p == null) return nvl;
