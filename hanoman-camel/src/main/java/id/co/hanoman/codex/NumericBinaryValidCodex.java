...
public class NumericBinaryValidCodex extends BinaryCodex {
	...
	public NumericBinaryValidCodex(CodexFactory factory) {
		super(factory);
	}
    ...
    public Object decode(CodexContext ctx, Object msg) {
    	...
    	try {
    		...
    		ctx.getValueHandler().setCodexError(ctx, msg, getName(), "Error "+getFullName()+" "+U.dump(b)+" "+ex.getMessage());
    		if (ignoreParseError) {
    			return msg;
    		} else {
    			throw new RuntimeException(ex.getMessage(), ex);
	...
	public void encode(CodexContext ctx, JsonObject msg) throws Exception {
		String data = ctx.getValueHandler().getCodexValue(ctx, msg, getName(), String.class);
		encode(ctx, data);
	}
